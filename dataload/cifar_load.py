...
class CIFARDataset(Dataset):
    def __init__(self, subset = False, sub_selection_technique = None,  percentage = 0.1, 
        ...
        if not subset:
            ...
            if os.path.exists(f'data/cifar_{sub_selection_technique}_{int(percentage*100)}.txt'):
                self.idxs = np.loadtxt(f'data/cifar_{sub_selection_technique}_{int(percentage*100)}.txt', dtype=int).tolist()
            else:
                self.idxs = self.subset_selection(sub_selection_technique, percentage, bs)
                np.savetxt(f'data/cifar_{sub_selection_technique}_{int(percentage*100)}.txt', np.array(self.idxs), fmt='%d')
    ...
    def subset_selection(self, sub_selection_technique, percentage, bs):
        ...
        with torch.no_grad():
            ...
            complete_dataset = CIFARDataset(transform = transform)
